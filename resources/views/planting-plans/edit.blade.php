...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50 p-6">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            <h1 class="text-3xl md:text-4xl font-black text-slate-900">✏️ Редактирование плана</h1>
            <p class="text-slate-500 mt-2 text-sm md:text-base">{{ $plan->name }}</p>
        </div>
    ...
    <form action="{{ route('planting-plans.update', $plan->id) }}" method="POST" class="space-y-6">
        @csrf @method('PUT')
        ...
        <div class="bg-white rounded-2xl p-6 shadow-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="number" name="planting_rate" id="plantingRate" value="{{ old('planting_rate', $plan->planting_rate) }}" required min="1"
                           class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-emerald-500">
                </div>
                ...
                <div>
                    ...
                    <div id="totalQuantityDisplay" class="px-4 py-3 rounded-xl bg-emerald-100 text-emerald-700 font-bold">
                        {{ number_format($plan->total_quantity, 0, ',', ' ') }}
                    </div>
                </div>
            </div>
        </div>
...
const existingBedColors = {!! json_encode($bedColors) !!};
const existingFlowerBedIds = {!! json_encode($plan->flowerBeds->pluck('id')->toArray()) !!};

console.log('existingBedColors:', existingBedColors);
console.log('existingFlowerBedIds:', existingFlowerBedIds);

function addFlowerBed(flowerBedId = null) {
    console.log('addFlowerBed called with flowerBedId:', flowerBedId);
    ...
    flowerBedsList.forEach(function(bed) {
        const selected = bed.id == flowerBedId ? 'selected' : '';
        flowerBedOptions += '<option value="' + bed.id + '" data-area="' + bed.area + '" data-name="' + bed.full_name + '" ' + selected + '>' + bed.full_name + ' (' + bed.area + ' м²)</option>';
    });

    let colorInputs = '';
    for (const type in colorTypes) {
        const data = colorTypes[type];
        const existingQty = (flowerBedId && existingBedColors[flowerBedId] && existingBedColors[flowerBedId][type]) ? existingBedColors[flowerBedId][type] : 0;
        colorInputs += '<div><label class="text-xs text-slate-500 block mb-1">' + data.icon + ' ' + data.name + '</label>';
        colorInputs += '<input type="number" name="flower_beds[' + rowId + '][colors][' + type + ']" data-color="' + type + '" class="color-input w-full px-2 py-2 rounded-lg border border-slate-200 text-sm text-center" placeholder="0" min="0" value="' + existingQty + '" oninput="updateBedTotal(this, ' + rowId + ')"></div>';
    }
    ...
    if (flowerBedId) {
        const select = row.querySelector('.bed-select');
        const option = select.querySelector('option[value="' + flowerBedId + '"]');
        if (option) {
            const area = option.getAttribute('data-area') || 0;
            row.querySelector('.bed-area').value = parseFloat(area).toFixed(2);
        }
        updateBedTotalForRow(row);
    }
...
function updateBedTotalForRow(row) {
    let total = 0;
...
function updateSummary() {
    ...
    document.getElementById('totalAreaDisplay').textContent = totalArea.toFixed(2).replace('.', ',');
...
document.getElementById('addFlowerBedBtn').addEventListener('click', function() {
    addFlowerBed(null);
});

existingFlowerBedIds.forEach(function(flowerBedId) {
    addFlowerBed(flowerBedId);
});

if (existingFlowerBedIds.length === 0) {
    addFlowerBed(null);
}
